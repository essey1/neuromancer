"""
Tests for prepare_gpphs_data() in neuromancer/dataset.py
...
def test_basic_with_control():
    ...
    (train, dev, test), dims = prepare_gpphs_data(x, x_dot, u, batch_size=32)
...
def test_1d_inputs_auto_promoted():
    ...
    T = 300
    x     = np.random.randn(T)   # shape (300,)
    x_dot = np.random.randn(T)
    u     = np.random.randn(T)

    (train, _, _), _ = prepare_gpphs_data(x, x_dot, u, batch_size=16)
    batch = next(iter(train))
...
def test_autonomous_no_u():
    ...
    (train, _, _), dims = prepare_gpphs_data(x, x_dot, u=None, batch_size=32)
    batch = next(iter(train))
...
def test_custom_split_ratio():
    ...
    (train, dev, test), _ = prepare_gpphs_data(x, x_dot, split_ratio=[60.0, 20.0])
...
def test_default_split_thirds():
    ...
    (train, dev, test), _ = prepare_gpphs_data(x, x_dot)

    total = len(train.dataset) + len(dev.dataset) + len(test.dataset)
    assert total == T, f"Split lost samples: {total} != {T}"
    # each split should be roughly T//3
...
def test_output_dtype_float32():
    ...
    (train, _, _), _ = prepare_gpphs_data(x, x_dot)
    batch = next(iter(train))
...
if __name__ == '__main__':
    print("Running prepare_gpphs_data tests...\n")
